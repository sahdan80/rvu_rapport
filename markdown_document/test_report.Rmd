...
#Producerar brödtext.
text <- paste(mest_frekvent_färdmedel," är det vanligaste färdmedlet i ",rapportområde," och står för ",round(mest_frekvent_andel)," procent av alla resorna.",sep="")
...
```{r område-diagram, fig.align='center', fig.hight = 3,fig.width=8, echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per område."}
#skapa Percent stacked barchart
trip_person_mode_plot<-trip_person_mode %>%
  gather(fard,andel,Bil:Övrigt,factor_key=TRUE) %>%
  rename("Färdmedel" = fard,"Andel" = andel)
trip_person_mode_plot$Andel<-round(trip_person_mode_plot$Andel,digit=0)
omrade_plot <- ggplot(data = trip_person_mode_plot, aes(fill =forcats::fct_rev(factor(Färdmedel)), x = Andel, y = forcats::fct_rev(factor(Område)),label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=trip_person_mode_plot[trip_person_mode_plot$Andel>2,],size = 2, position = position_fill(vjust = 0.2))+labs(x="Andel (%)",y="Område",fill="Färdmedel")
omrade_plot
```
...
<Diagram med färdmedelsandelar per kön>
...
```{r gender-diagram, fig.align='center', fig.hight = 3,fig.width=8, echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per kön."}
#beräkna andel
...
temp_plot<-trip_mode_gender_andel %>%
  gather(Färdmedel,Andel,Bil:Övrigt,factor_key=TRUE)
temp_plot$Andel<-round(temp_plot$Andel,digit=0)
gender_plot <- ggplot(data = temp_plot, aes(fill =forcats::fct_rev(factor(Färdmedel)), x = Andel, y = forcats::fct_rev(factor(Kön)),label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=temp_plot[temp_plot$Andel>2,],size = 2, position = position_fill(vjust = 0.2))+labs(x="Andel(%)",y="Kön",fill="Färdmedel")
gender_plot
```
...
<Diagram med färdmedelsandelar per ålder>
...
```{r age-diagram, fig.align='center', fig.hight=3, fig.width = 8, echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per åldersgrupp."}
#beräkna andel
...
temp_plot<-trip_age_andel %>%
  gather(Färdmedel,Andel,Bil:Övrigt,factor_key=TRUE)
temp_plot$Andel<-round(temp_plot$Andel,digit=0)
age_plot <- ggplot(data = temp_plot, aes(fill =forcats::fct_rev(factor(Färdmedel)), x = Andel, y = forcats::fct_rev(factor(Ålder)),label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=temp_plot[temp_plot$Andel>2,],size = 2, position = position_fill(vjust = 0.2))+labs(x="Andel(%)",y="Ålder",fill="Färdmedel")
age_plot
```
...
<Diagram med färdmedelsandelar per ärende>
...
```{r purpose-mode-diagram, fig.align='center', fig.hight = 3,fig.width=8, echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per ärende."}
#beräkna andel
temp_plot<-trip_purpose_mode
colnames(temp_plot)[which(names(temp_plot) == "Kollektiv buss")] <- "Buss"
colnames(temp_plot)[which(names(temp_plot) == "Kollektiv tåg")] <- "Tåg"
...
temp_plot<-temp_plot %>%
  gather(Färdmedel,Andel,Buss:Övrigt,factor_key=TRUE)
temp_plot$Andel<-round(temp_plot$Andel,digit=0)
purpose_plot <- ggplot(data = temp_plot, aes(fill =forcats::fct_rev(factor(Färdmedel)), x = Andel, y = forcats::fct_rev(factor(Ärende)),label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=temp_plot[temp_plot$Andel>10,],size = 2, position = position_fill(vjust = 0.3))+labs(x="Andel(%)",y="Ärende",fill="Färdmedel")
purpose_plot
```
...
<Histogram med resavstånd per färdmedel>
...
```{r distans-mode-diagram, fig.align='center', fig.hight = 3,fig.width=8, echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per avstånd klass."}
rvu_avs <- rvu %>%
  filter(!is.na(avstand)) %>%
  rename(Färdmedel = fardmedel.kat) %>%
  select(respondentid,Färdmedel,avstand,Område,Område_kod)
rvu_avs$km<-ceiling(rvu_avs$avstand/1000)
rvu_avs$kmklass<-ifelse(rvu_avs$km>15,16,rvu_avs$km)
rvu_avs$antal<-1
agg_rvu_avs <-
...
temp_plot <- agg_rvu_avs %>%
  mutate("Bil" = 100*Bil / Total,
         ...
         "Gång" = 100*Gång / Total) %>%
  select(-Total) %>%
  gather(Färdmedel,Andel,Bil:Tåg,factor_key=TRUE)
temp_plot$Andel<-round(temp_plot$Andel,digit=0)
avstand_plot <- ggplot(data = temp_plot, aes(fill =forcats::fct_rev(factor(Färdmedel)), y = Andel, x = factor(avstand),label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=temp_plot[temp_plot$Andel>2,],size = 2, position = position_fill(vjust = 0.2))+labs(y="Andel(%)",x="Avstånd (km)",fill="Färdmedel")
avstand_plot
```
