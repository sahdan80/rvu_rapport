...
```{r setup, include=FALSE}
  ...
  "knitr", "kableExtra", "tidyverse", "readxl", "bookdown", "pxweb", "scales", "stringi", "leaflet", "sf", "mapview", "processx", "tinytex")
...
reslangd_barplot <- ggplot(reslangd_per_fardmedel, aes(factor(Färdmedel, level = ordning_fardmedel), perc_reslangd, fill = factor(Färdmedel, level = ordning_fardmedel))) +
    ...
    scale_fill_manual(values=rev(palett)) +
    scale_y_continuous(labels = scales::percent) +
    geom_text(aes(label=percent(perc_reslangd, 0.1)),size = 3, position=position_dodge(width=0.9), vjust=-0.25) + #etiketter
    labs(
        x = "Färdmedel",
...
omrade_plot <- ggplot(data = trip_person_mode_plot, aes(fill =forcats::fct_rev(factor(fard,level = ordning_fardmedel)), x = andel, y = forcats::fct_rev(factor(Omrade,level = trip_mode$Omrade)),label =andel)) + 
...
<Exempeltext> <Använd trip_purpose_mode_df>
Resor till bostaden utgör en stor del av alla resor. I denna mätning är andelen X procent och andelen som gjordes till arbete var X procent. Av de resor som gjordes av de som har haft arbete som ärende för resan är X procent bilresor, X procent kollektivtrafiksresor och X procent cykelresor.

Bil valdes i högst utsträckning för ärende X, med X %. För ärende Y användes dock bil i lägst utsträckning (Y %), medan fler resor istället gjordes med Z (Z %).
...
<Exempeltext> <baserat på reslangd_per_omrade_ny>
...
>>>>>>> ca282ef4437b39c8aba1b6cb96175a6f4c71122f
...
```
![Karta av alla områden i RVU:n]("karta_omraden.png")
