...
header-includes:
  ...
  - \makeatother
  # Har inte fått detta att fungera än:
  # if (!require("pacman")) install.packages("pacman")
  # pacman::p_load(booktabs)
  # pacman::p_load(longtable)
  # pacman::p_load(array)
  # pacman::p_load(multirow)
  # pacman::p_load(wrapfig)
  # pacman::p_load(float)
  # pacman::p_load(colortbl)
  # pacman::p_load(pdflscape)
  # pacman::p_load(tabu)
  # pacman::p_load(threeparttable)
  # pacman::p_load(threeparttablex)
  # pacman::p_load(normalem)
  # pacman::p_load(makecell)
  # pacman::p_load(xcolor)
  # pacman::p_load(document)
...
#Funktion för att räkna upp områden i en slice
upprakning<-function(extremer,max_antal_uppräknade){
  antal_uppräknade <- min(nrow(extremer),max_antal_uppräknade) #som minst räknas alla områden upp
  text_uppräkning <- paste(extremer$Omrade[1:(antal_uppräknade)], collapse = ', ') #sträng med områden
...
pers <- mutate(pers,bostad_deso_kod = bostad_deso) #Flag: Den här ska tas bort när Björn uppdaterat indatatabellerna.
...
pers <- pers %>%
  filter(Rapportomrade %in% rapportomrade) #Flag: Hårdkodat.. Ska ändras om vi gör en regionrapport...
rvu <- rvu %>%
...
rvu_yr <- as.character(as.numeric(rvu_yr)-1)
...
## Antal resor
Antalet resor i `r (rapportomrade_text)` uppgick i X resor per dygn...

## Färdmedelsfördelning

Färdmedelsfördelningen visar att X är det vanligaste färdmedlet.
<Osäkert hur det ska visualiseras, diagram?>

```{r fardmedelsfordelning-diagram, echo = FALSE, results = "asis", fig.cap = "Resornas färdmedelsfördelning per område."}
# Stacked bar plot, fast bara med en för hela området...
# fardmedelsandelar <- rvu %>%
#   select(fardmedel.kat) %>%
#   filter(!is.na(fardmedel.kat)) %>%
#   group_by(fardmedel.kat) %>%
#   summarize(antal = n()) %>%
...
#   select(ordning_fardmedel)
# 
# 
# #Percent stacked barchart
# fardmedelsandelar_plot<-fardmedelsandelar %>%
#   mutate(across(where(is.numeric), ~ . * 100))  %>%
#   gather(fard,andel,Bil:Annat,factor_key=TRUE) %>% #Flag: Bil_Annat borde inte vara hårdkodat...
#   mutate(andel=round(andel)) %>%
#   mutate(Omrade = "Totalt")
# 
# omrade_plot <- ggplot(data = fardmedelsandelar_plot, aes(fill =forcats::fct_rev(factor(fard)), x = andel, y = forcats::fct_rev(factor(Omrade)),label =andel)) + 
...
purp_barplot <- ggplot(test, aes(fardmedel.kat, antal, fill = fardmedel.kat)) +
    geom_bar(stat = "identity") +
    rvu_theme() +
    coord_cartesian(xlim = c(1,nrow(test))) +
    scale_fill_manual(values=palett) +
    geom_text(aes(label=antal),size = 3, position=position_dodge(width=0.9), vjust=-0.25) + #etiketter
    labs(
        x = "Ärende",
        y = "Antal svar",
...
<Visa Totalen i slutet av tabellen i avsnitt 3.5?>
...
if (nrow(extremer)>1){ #om det finns extremer
  ...
  text <- paste(text,text_uppräkning,", där andelen svarande var upp emot ",round(extremer$Svarsfrekvens[1])," %. ",sep="")
    ...
    text <- paste(text,"Högst andel svarande återfinns i ",svarsgrad_max$Omrade,", där andelen svarande var ",round(svarsgrad_max$Svarsfrekvens)," %. ",sep="")
}
...
if (nrow(extremer)>1){ #om det finns extremer
  ...
  text <- paste(text,text_uppräkning,", där andelen svarande var upp emot ",round(extremer$Svarsfrekvens[1])," %. ",sep="")
    ...
    text <- paste(text,"Lägst andel svarande återfinns i ",svarsgrad_min$Omrade,", där andelen svarande var nära ",round(svarsgrad_min$Svarsfrekvens)," %. ",sep="")
  }
...
Ur undersökningen har de `r (antal_individer)` respondenter som bor i `r (rapportomrade_text)` valts ut. I Tabell \@ref(tab:svarsfrekvens-table) återfinns befolkning, antal respondenter och andelen svarande av befolkningen. Respondenternas kön och ålder per område finns redovisade i Tabell \@ref(tab:repondant-table).
...
extremer <- extremer_höga(lista,"Körkort (%)") #Extremer som ska behandlas

#Skapar text
if (nrow(extremer)>1){ #om det finns extremer
  text <- paste(text,"Särskilt högt var körkortsinnehavet bland människor i ")
  #Uppräkning av områden
  text_uppräkning <- upprakning(extremer,max_antal_uppräknade)
  text <- paste(text,text_uppräkning,", där andelen var upp emot ",round(extremer$col[1])," %. ",sep="")
  } else {
    max <- slice(lista,which.max(col))
    text <- paste(text,"Högst körkortsinnehav återfanns i ",max$Omrade,", med ",round(max$col)," %. ",sep="")
  }

extremer <- extremer_låga(lista,col)
if (nrow(extremer)>1){ #om det finns extremer
  text <- paste(text,"Särskilt låg andel körkortsinnehavare återfanns i ")
  #Uppräkning av områden
  text_uppräkning <- upprakning(extremer,max_antal_uppräknade)
  text <- paste(text,text_uppräkning,", där andelen var ner emot ",round(extremer$col[1])," %. ",sep="")
  } else {
    min <- slice(lista,which.min(col))
    text <- paste(text,"Lägst var andelen med körkort i ",min$Omrade,", där andelen var nära ",round(min$col)," %. ",sep="")
    }

```

I `r (rapportomrade_text)` hade `r round(körkort_totalt)` procent av respondenterna körkort och `r round(biltillgång_totalt)` procent tillgång till bil. `r (text)`De som hade tillgång till bil varierade mellan  `r round(min(korkort_bil$"Biltillgång (%)"))` och `r round(max(korkort_bil$"Biltillgång (%)"))` procent. I Tabell \@ref(tab:korkort-bil-table) framgår körkortsinnehav och biltillgång för respektive område.
<OBS siffrorna i tabellen nedan är fel. Det finns en fungerande version som ska återskapas>
...
En genomsnittlig dag genomfördes `r round(num_trips_all$"Antal resor",0)` resor i `r (rapportomrade_text)` av personer 15–85 år bosatta inom det undersökta utredningsområdet. I Tabell \@ref(tab:num-trips-table) framgår andel personer som rest och deras antal resor per dag redovisat per kön och ålderskategori.
...
x <- kbl(num_trips_rep, digits = 1, longtable = T, booktabs = T, linesep = "",caption = "Antal resor per dag per kön",format.args = list( big.mark = " ")) %>%
...
```{r trips_person_mode_text, echo=FALSE, include=FALSE}
#Flag: Se över detta! Hårdkodade antal kolumner, och behövs de verkligen?
#Räknar ut vanligaste färdmedlet i respektive område
trip_person_mode_max <- trip_mode %>%
  mutate('max' = names(trip_mode)[2:7][max.col(trip_mode[2:7], "first")])
...
mest_frekvent_andel <- pull(trip_mode[nrow(trip_mode),mest_frekvent_färdmedel])*100
...
trip_person_mode_extremer_höga <- extremer_höga(trip_mode,mest_frekvent_färdmedel)
trip_person_mode_extremer_låga <- extremer_låga(trip_mode,mest_frekvent_färdmedel)


#Producerar brödtext.
text <- paste(tolower(mest_frekvent_färdmedel)," är det vanligaste färdmedlet i ",rapportomrade_text," och står för ",round(mest_frekvent_andel)," procent av alla resorna. ",sep="")
...
if (nrow(avvikande_vanligaste_färdmedel) > 0) {
  ...
  text_uppräkning <- paste(avvikande_vanligaste_färdmedel$Omrade[1:(antal_uppräknade)], collapse = ', ') #sträng med områden
  text_uppräkning <- stri_replace_last_fixed(text_uppräkning, ', ', ' och ') #sätter "och" i slutet av uppräkningen
...
if (nrow(extremer)>1){ #om det finns extremer
  ...
  text_uppräkning <- paste(extremer$Omrade[1:(antal_uppräknade)], collapse = ', ') #sträng med områden
  text_uppräkning <- stri_replace_last_fixed(text_uppräkning, ', ', ' och ') #sätter "och" i slutet av uppräkningen
  text <- paste(text,text_uppräkning,", där andelen var upp emot ",round(extremer$Bil[1]*100)," %. ",sep="")
  } else {
    tabell_max <- slice(tabell,which.max(Bil))
    text <- paste(text,"Högst andel återfanns i ",tabell_max$Omrade,", med ",round(tabell_max$Bil*100)," %. ",sep="")
}
...
if (nrow(extremer)>1){ #om det finns extremer
  ...
  text_uppräkning <- paste(extremer$Omrade[1:(antal_uppräknade)], collapse = ', ') #sträng med områden
  text_uppräkning <- stri_replace_last_fixed(text_uppräkning, ', ', ' och ') #sätter "och" i slutet av uppräkningen
  text <- paste(text,text_uppräkning,", där andelen var ner emot ",round(extremer$Bil[1]*100)," %. ",sep="")
  } else {
    tabell_min <- slice(tabell,which.min(Bil))
    text <- paste(text,"Lägst var andelen i ",tabell_min$Omrade,", där endast ",round(tabell_min$Bil*100)," % av resorna gjordes med ",tolower(mest_frekvent_färdmedel),". ",sep="")
  }
...
if (mest_frekvent_färdmedel != "Cykel") {
  ...
  text <- paste(text," Andelen cyklister var i genomsnitt ",round(andel_resor_cykel*100)," %. Högst andel cyklister återfanns i ",pull(andel_resor_cykel_max["Omrade"])," med en andel på ",round(pull(andel_resor_cykel_max["Cykel"]*100))," %. ",sep="")
}

```

I Figur \@ref(fig:omrade-diagram) framgår att `r (text)`
...
trip_person_mode_plot<-trip_mode %>%
  ...
  gather(fard,andel,Bil:Annat,factor_key=TRUE) %>% #Flag: Bil_Annat borde inte vara hårdkodat...
  mutate(andel=round(andel))
omrade_plot <- ggplot(data = trip_person_mode_plot, aes(fill =forcats::fct_rev(factor(fard)), x = andel, y = forcats::fct_rev(factor(Omrade)),label =andel)) + 
  theme(panel.background = element_blank(),legend.position="bottom") +
  geom_bar(position = "fill", stat = "identity")+
  geom_text(aes(label=ifelse(andel>3,andel," ")),size = 3, position = position_fill(vjust = 0.5)) + #etiketter
  scale_fill_manual(values=palett) + #färger
...
<Exempeltext>
Andelen kvinnor och män  i undersökningen är ungefär lika stora, X respektive Y procent. Hur de reser brukar skilja sig åt, så även i denna mätning. X åker mer bil men har färre förflyttningar med färdmedel Y.
...
trip_mode_gender <- rvu %>%
  ...
  # left_join(select(scb_befolkning, c(Omrade_kod,Män,Kvinnor)), by = c("Omrade_kod" = "Omrade_kod")) %>%
  # mutate(Befolkning = (kon == "Man")*Män + (kon == "Kvinna")*Kvinnor)
  # group_by(Omrade,kon) %>%
  # summarize("Befolkning" = if_else(kon == "Man", sum(Män), sum(Kvinnor))) %>%
  # ungroup()
  summarize(antal = n()) %>%
  spread(fardmedel.kat, antal, fill=0) %>%
  mutate(Totalt = rowSums(across(where(is.numeric)))) %>%
...
<Exempeltext>
Åldersgruppen X år utgör X procent, gruppen Y år Y procent och åldersgruppen Z år Z procent av den undersökta befolkningen. I diagrammet nedan jämförs färdsättsanvändning i olika åldersgrupper. Färdsättet övrigt redovisas inte.
...
<Exempeltext>
I åldern 12–84 år är X procent förvärvsarbetande, X procent studerande, X procent är pensionärer och gruppen övriga utgör X procent av samtliga. Gruppen övrig sysselsättning består huvudsakligen av föräldralediga, arbetslösa, värnpliktiga och långtidssjukskrivna. Av de resor som genomförs av förvärvsarbetande är X procent bilresor, X procent bussresor, X procent cykelresor och X procent förflyttningar till fots och X procent övrigt. Diagrammet visar att även att X av de som studerar cyklar och att ungefär var X pensionär cyklar. Gruppen övrigt har störst andel som förflyttat sig till fots, X procent.
...
<Exempeltext>
Resor till bostaden utgör en stor del av alla resor. I denna mätning är andelen 38 procent och andelen som gjordes till arbete var 20 procent. Av de resor som gjordes av de som har haft arbete som ärende för resan är 40 procent bilresor, 14 procent bussresor, 36 procent cykelresor och sju procent är gångförflyttningar och tre procent övrigt.
...
od_matrix <- rvu %>%
    filter(start_tatort_namn %in% rapportomrade & stop_tatort_namn %in% rapportomrade) %>%
...
od_matrix <- od_matrix %>%
  top_n(minsta_antal) %>%
    bind_rows(
      od_matrix %>%
        filter(!(start_deso %in% top_n(od_matrix,minsta_antal)$start_deso)) %>%
        summarise_if(is.numeric, sum) %>%
        mutate(start_deso = "Övriga områden")
    ) %>%
  bind_rows(summarize(.,
                      across(where(is.numeric), sum),
                      across(where(is.character), ~ "Totalt")))
...
lista_med_omraden <- pull(od_matrix,start_deso)
od_matrix <- od_matrix %>%
  select(start_deso,lista_med_omraden[1:minsta_antal],Totalt) %>%
  rename("Från område" = start_deso) %>%
...
kable(od_matrix_perc, caption = "Fördelning av resorna inom tätorten.", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
...
```
### Reserelationer utanför `r (rapportomrade_rubrik)`
...
od_matrix <- od_matrix %>%
  top_n(minsta_antal) %>%
    bind_rows(
      od_matrix %>%
        filter(!(start_tatort_namn %in% top_n(od_matrix,minsta_antal)$start_tatort_namn)) %>%
        summarise_if(is.numeric, sum) %>%
        mutate(start_tatort_namn = "Övriga områden")
    ) %>%
  bind_rows(summarize(.,
                      across(where(is.numeric), sum),
                      across(where(is.character), ~ "Totalt")))
...
od_matrix <- od_matrix %>%
  select(start_tatort_namn,lista_med_tätorter[1:minsta_antal],Totalt) %>%
  rename("Från tätort" = start_tatort_namn) %>%
  mutate("Övriga områden" = -rowSums(select_if(., is.numeric), na.rm = TRUE)+2*Totalt) %>%
  relocate(Totalt, .after = "Övriga områden")

#Omvandlar till dataframe
od_matrix <- as.data.frame(od_matrix) #Omvandalr till dataframe
rownames(od_matrix) <- od_matrix$"Från tätort" #Sätter från som indexkolumn
od_matrix <- select(od_matrix,!c("Från tätort")) #Tar bort dublettkolumn
...
kable(od_matrix_perc, caption = "Resor av invånare i Bålsta till och/eller från andra tätorter.", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
  row_spec(nrow(od_matrix_perc), bold = T) %>% #gör Total-raden fetstilt
  row_spec(0, bold = T, angle = 45) %>% #gör rubrikraden fetstilt)
...
#Antal svar perärende... Ska inte användas
