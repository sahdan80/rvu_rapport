...
pers_group <- pers %>%
  select(BostadTatortNamn, kon, alder) %>%
  group_by(BostadTatortNamn) %>%
  ...
  rename('Tätort' = BostadTatortNamn)
...
trip_person_mode <- pers %>%
  select(respondentid, BostadTatortNamn)%>%
  left_join(mode_per_pers, by = c("respondentid" = "respondentid")) %>%
  filter(!is.na(BostadTatortNamn), made_trip) %>%
  group_by(BostadTatortNamn) %>%
