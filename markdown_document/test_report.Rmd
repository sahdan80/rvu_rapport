...
```{r område-diagram, fig.align='center', echo = FALSE, results = "asis", fig.cap = "Resor fördelning avseen färdsätt per område."}
...
omrade_plot <- ggplot(data = trip_person_mode_plot, aes(fill =Färdmedel, y = Andel, x =  Område,label =Andel)) + geom_bar(position = "fill", stat = "identity")+geom_text(data=trip_person_mode_plot[trip_person_mode_plot$Andel>2,],size = 3, position = position_fill(vjust = 0.5))+coord_flip()
omrade_plot
```
