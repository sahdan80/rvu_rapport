...
Under <tidsperiod> besvarade antal_personer kollektivtrafiksbarometern i `r (rapportomrade_text)` i åldern 15–84 år. Syftet med undersökningen är att kartlägga hur resandet ser ut en genomsnittlig vardag <tidsperiod>. Undersökningen omfattar enbart persontransporter och har genomförts 
som en enkätundersökning. Resultaten är viktade med avseende på urvalsområde, kön och ålder för att motsvara hela befolkningen i den undersökta åldersgruppen.
...
## Antal resor
Antalet resor per dygn i `r (rapportomrade_text)` uppgick i `r round(num_trips_all$"Antal resor")`. I genomsnitt gjorde `r round(num_trips_all$"Andel personer som rest (%)")` procent åtminstone en resa per dag. I åldersgruppen `r (maxgrupp_andel_rest)` var andelen som rest `r round(maxgrupp_andel_rest_varde)` procent, vilket är mer än andra åldersgrupper.
