...
Under `r paste(min(pers$ar),ifelse(min(pers$ar)!=max(pers$ar),paste("-",max(pers$ar),sep = ""),""),sep = "")` besvarades kollektivtrafiksbarometern av `r (length(unique(rvu$respondentid)))` 15-85-åringar boende i `r (rapportomrade_text)`. Syftet med undersökningen är att kartlägga hur resandet ser ut en genomsnittlig vardag `r max(pers$ar)`. Undersökningen omfattar enbart persontransporter och har genomförts som en enkätundersökning. Resultaten är viktade med avseende på urvalsområde, kön och ålder för att motsvara hela befolkningen i den undersökta åldersgruppen.
...
#Producerar brödtext.
text <- paste(tolower(mest_frekvent_färdmedel)," är det vanligaste färdmedlet i ",rapportomrade_text," och står för ",round(mest_frekvent_andel*100)," procent av alla resorna. ",sep="")
...
Tabell: Antal resor per dag per resenärsgrupp : Andel personer som rest (%) : Kvinna/Man
...
text <- paste("Andelen svarande av befolkningen (15 år och äldre) var i genomsnitt ",round(svarsgrad_totalt*100)/100," %. ")
...
if (avvikelse > 0.8) {
  text <- paste("Andelen som rest bland ",kön1a," är högre än bland ",kön2a,". ",sep="")
  konjunktion <- "dock"
  konjunktion2 <- "dessutom"
} else {
    text <- paste("Andelen som rest bland ",kön1a," och ",kön2a," var i samma storleksordning. ",sep="")
...
if (avvikelse>0.032) {
  text <- paste(text, "Bland de som reser var ",konjunktion2," antalet resor per dag större bland ",kön1b," än bland ",kön2b,". ",sep="")
  } else {
    text <- paste(text, "Bland de som reser var ",konjunktion," antalet resor per dag snarlikt mellan ",kön1b," och ",kön2b,". ",sep="")
    konjunktion <- "heller "
}
...
`r text` I åldersgruppen `r (maxgrupp_andel_rest)` var andelen som rest `r round(maxgrupp_andel_rest_varde)` procent, vilket är mer än övriga åldersgrupper.
...
if (avvikelse > 0.02) {
  text <- paste(text,"Vad gäller färdmedelsandelar var bil populärare bland kvinnor än bland män. ",sep = "")
} else if (avvikelse < -0.02) {
  text <- paste(text,"Vad gäller färdmedelsandelar var bil populärare bland män än bland kvinnor. ",sep = "")
} else {
  text <- paste(text,"Vad gäller färdmedelsandelar var bil ungefär lika populärt bland män som bland kvinnor. ",sep = "")
}
...
if (avvikelse > 0.02) {
  text <- paste(text,"Kollektivtrafik var populärare bland kvinnor än bland män. ",sep = "")
} else if (avvikelse < -0.02) {
  text <- paste(text,"Kollektivtrafik var populärare bland män än bland kvinnor. ",sep = "")
} else {
  text <- paste(text,"Kollektivtrafik var ungefär lika populärt bland män som bland kvinnor. ",sep = "")
}

avvikelse <- trip_mode_gender_andel_df["Kvinna","Cykel"] - trip_mode_gender_andel_df["Man","Cykel"]
if (avvikelse > 0.02) {
  text <- paste(text,"Cykel var populärare bland kvinnor än bland män. ",sep = "")
} else if (avvikelse < -0.02) {
  text <- paste(text,"Cykel var populärare bland män än bland kvinnor. ",sep = "")
} else {
  text <- paste(text,"Cykel var ungefär lika populärt bland män som bland kvinnor. ",sep = "")
}
...
```
<Exempeltext>
Andelen män och kvinnor i undersökningen är `r round(andel_man)` respektive `r round(andel_kvinnor)` %. Hur de reser brukar skilja sig åt. X åker mer bil men har färre förflyttningar med färdmedel Y.andel_kvinnor
