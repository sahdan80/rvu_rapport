...
kbl(trip_mode, caption = "Antal resor per färdmedel och område", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  row_spec(nrow(trip_mode), bold = T) %>% #gör Total-raden fetstilt
  row_spec(0, bold = T) #gör rubrikraden fetstilt)
```
...
# kbl(trip_mode_gender, caption = "Färdmedel per kön", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
#   kable_styling(latex_options = c("striped", "HOLD_position")) %>%
...
# kbl(trip_age, caption = "Färdmedel per åldersgrupp", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
#   kable_styling(latex_options = c("striped", "HOLD_position")) %>%
#   row_spec(nrow(trip_mode), bold = T) %>% #gör Total-raden fetstilt
#   row_spec(0, bold = T) #gör rubrikraden fetstilt
```
...
```{r od_matrix_inom, echo=FALSE, include = FALSE}
...
lista_med_tätorter <- pull(od_matrix_inom,start_deso)
od_matrix_inom <- od_matrix_inom %>%
...
od_matrix_inom_perc <- as.data.frame(od_matrix_inom) #konverterar till dataframe
rownames(od_matrix_inom_perc) <- od_matrix_inom_perc$"Från tärort" #Skapar indexkolumn
od_matrix_inom_perc <- select(od_matrix_inom_perc,!c("Från tärort")) #Tar bort gammal kolumn
od_matrix_inom_perc <- rbind(od_matrix_inom_perc[1:minsta_antal-1,])
od_matrix_inom_perc <- round(od_matrix_inom_perc/od_matrix_inom_perc["Totalt","Totalt"]*100)
...
kable(od_matrix_inom_perc, caption = "Fördelning av resorna inom tätorten.", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
...
# #Sparar OD-matris som csv
# write.csv(od_matrix_extern, "od_matrix_extern.csv")

#Filtrerar ut populäraste områden på rader samt skapar total och övriga områden
od_matrix_extern <- od_matrix_extern %>%
    mutate("Totalt" = rowSums(select_if(., is.numeric), na.rm = TRUE)) %>%
    arrange(desc(Totalt)) %>%
    bind_rows(summarize(.,
                      across(where(is.numeric), sum),
...
#Skapar övrigt-rad
od_matrix_extern_övriga  <- 2*od_matrix_extern[minsta_antal, -1]- od_matrix_extern[minsta_antal+1,-1]
od_matrix_extern_övriga <- mutate(od_matrix_extern_övriga,start_tatort_namn="Övriga områden")
od_matrix_extern <- rbind(od_matrix_extern[1:minsta_antal-1,],od_matrix_extern_övriga,od_matrix_extern[minsta_antal,])

#Filtrerar bort kolumner
lista_med_tätorter <- pull(od_matrix_extern,start_tatort_namn)
od_matrix_extern <- od_matrix_extern %>%
...
#Skapar procent-matris
od_matrix_extern_perc <- as.data.frame(od_matrix_extern)
rownames(od_matrix_extern_perc) <- od_matrix_extern_perc$"Från tärort"
od_matrix_extern_perc <- select(od_matrix_extern_perc,!c("Från tärort"))
od_matrix_extern_perc <- rbind(od_matrix_extern_perc[0:minsta_antal+1,])
od_matrix_extern_perc <- round(od_matrix_extern_perc/od_matrix_extern_perc["Totalt","Totalt"]*100)

```

```{r od-matrix-extern-table, echo = FALSE, result="asis"}
kable(od_matrix_extern_perc, caption = "Resor av invånare i Bålsta till och/eller från andra tätorter.", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
  row_spec(0, bold = T, angle = 45) %>% #gör rubrikraden fetstilt)
  column_spec(1, bold = T, width = "2cm")
...
kable(trip_purpose_area, caption = "Resärende per område", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
...
kable(trip_purpose_area_percent, caption = "Resärende per område (procent)", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"),
                full_width = TRUE) %>%
  row_spec(0, bold = T, angle = 60) %>% #gör rubrikraden fetstilt
  column_spec(1,width = "2cm")
...
kable(drivers_license_area, caption = "Körkortsinnehav per område", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position")) %>%
...
kable(car_ownership_area, caption = "Bilinnehav per område", booktabs = T, linesep = "",format.args = list( big.mark = " ")) %>%
  kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  #row_spec(nrow(drivers_license_area), bold = T) %>% #gör Total-raden fetstilt
  row_spec(0, bold = T) #gör rubrikraden fetstilt)
```
